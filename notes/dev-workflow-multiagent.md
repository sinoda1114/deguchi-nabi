...
- リポジトリの**実体ディレクトリ `~/dev/deguchi-nabi` は「main 統合＋デプロイ専用」**。
...
git fetch origin
git worktree add ../deguchi-nabi-<topic> -b feat/<topic> origin/main
# 起点ズレ確認（0 であること）
git -C ../deguchi-nabi-<topic> rev-list --count origin/main..HEAD

# 一覧 / 後片付け
git worktree list
git worktree remove ../deguchi-nabi-<topic>   # マージ後に撤去
```

- 命名規則: `feat/<topic>` `fix/<topic>` `chore/<topic>`。worktree dir は `../deguchi-nabi-<topic>`。
- **同じファイルを2つの worktree で同時編集しない**（役割境界を守る）。
- worktree は本体 repo の `node_modules` を symlink すると検証が速い（`ln -s ../deguchi-nabi/node_modules node_modules`）。
