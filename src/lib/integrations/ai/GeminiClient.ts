...
async function callGemini(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(`[DIAG-TEMP] callGemini failed: status=${res.status} model=${model} keyLen=${apiKey.length} keyPrefix=${apiKey.slice(0, 4)} body=${bodyText.slice(0, 300)}`);
      return null;
    }
    ...
    console.error(`[DIAG-TEMP] callGemini threw: model=${model} keyLen=${apiKey.length} err=${err instanceof Error ? err.message : String(err)}`);
    return null;
  }
}
