...
describe("GeminiAiSdkClient", () => {
  ...
  describe("searchAndGenerateStructuredContent", () => {
    ...
    // pattern-b: 全箇所gemini-3.5-flash統一)で必須引数化した。呼び出し元が
    // バージョンを明示しない限りコンパイルが通らないようにするための変更。
    ...
    test("searchModel/extractionModelがいずれもgemini-3.5-flashで統一される(パターンB)", async () => {
      generateTextMock.mockResolvedValue({
        text: "検索結果テキスト",
        providerMetadata: {
          google: { groundingMetadata: { webSearchQueries: ["q1"] } },
      ...
      expect(generateObjectMock.mock.calls[0][0].model).toEqual({ modelId: "gemini-3.5-flash" });
    });
