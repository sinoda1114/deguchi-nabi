...
export async function generateUnifiedArrivalGuide(
  ...
  const destinationAccessPriorityNote = destinationHint
    ? `\n\n【出口の決め方(最優先)】\nまず「${destinationHint}」の公式サイト・予約ページ・グルメサイト等に「アクセス」「最寄り出口」として明記された出口名を検索してください。目的地自身が明言する出口名が見つかった場合は、それを最優先で採用してください(駅全体でよく使われる代表的な出口だからという理由だけで、目的地が明言していない出口を選ばないでください)。目的地自身の記載が見つからない場合のみ、次善策として徒歩距離が最短になる出口を駅構内図から判断してください。`
    : "";
