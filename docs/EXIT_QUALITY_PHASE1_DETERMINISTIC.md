...
**合格線（変更禁止）**: ユーザーに **乗車位置 AND 改札 AND 出口** の三つがすべて具体値で出ること。いずれかが「確認できません」なら不合格。出口のみ / 改札のみ / 方角だけは不合格（部分表示は可、合格率に数えない）。PR #133 Option A（gate-only 成功化）は不採用。

内部スコア `isScoringBothHit` は **改札 AND 出口** のまま（Gemini `.final` スキップ判定）。乗車位置をここに足さない（足すとカタログ経路が死ぬ）。ユーザー Presence と内部スコアは別物。
